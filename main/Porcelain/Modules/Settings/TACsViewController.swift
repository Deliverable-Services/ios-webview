//
//  TACsViewController.swift
//  Porcelain
//
//  Created by Justine Angelo Rangel on 28/08/2019.
...
private struct ContentAttributedStringAppearance: AttributedStringAppearanceProtocol {
...
public final class TACsViewController: UIViewController {
  @IBOutlet private weak var titleLabel: UILabel!
  @IBOutlet private weak var contentTextView: DesignableTextView!
...
extension TACsViewController: NavigationProtocol {
}

extension TACsViewController: ControllerProtocol {
  public static var segueIdentifier: String {
    fatalError("TACsViewController segueIdentifier not set")
  }
  
  public func setupUI() {
    titleLabel.attributedText = "TERMS OF USE".attributed.add([.color(.gunmetal), .font(.openSans(style: .semiBold(size: 16.0)))])
    let appearance = ContentAttributedStringAppearance()
    var appearance2 = appearance
    appearance2.font = .openSans(style: .regular(size: 13.0))
    contentTextView.attributedText =
    """
1. TERMS OF USE
By downloading, browsing, accessing or using this Sample Store mobile application (“Porcelain”), you agree to be bound by these Terms and Conditions of Use. We reserve the right to amend these terms and conditions at any time. If you disagree with any of these Terms and Conditions of Use, you must immediately discontinue your access to the Mobile Application and your use of the services offered on the Mobile Application. Continued use of the Mobile Application will constitute acceptance of these Terms and Conditions of Use, as may be amended from time to time.
...
""".attributed.add(.appearance(appearance2))
    view.subviews.first?.bringSubview(toFront: titleLabel)
  }
...
public protocol TACsPresenterProtocol {
}

extension TACsPresenterProtocol where Self: ControllerProtocol & UIViewController {
  public func showTermsAndConditions() {
    let tacsViewController = UIStoryboard.get(.settings).getController(TACsViewController.self)
    present(NavigationController(rootViewController: tacsViewController), animated: true, completion: nil)
  }
}
