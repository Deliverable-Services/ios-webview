...
public final class HomeViewController: UIViewController, RefreshHandlerProtocol {
    ...
    public override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier ?? "").elementsEqual("WebViewSegue") {
            guard let url = sender as? String  else {
                return
            }
            
            let webView = segue.destination as! HomeWebViewController
            url.contains("shop") ? (webView.navigationItem.title = "Products") : (webView.navigationItem.title = "Treatments")
    ...
    @IBAction private func productsTapped(_ sender: Any) {
        self.setupWebView(url: "https://porcelainskin.com/shop/")
    }
    
    @IBAction private func treatmentsTapped(_ sender: Any) {
        self.setupWebView(url: "https://porcelainskin.com/treatments/")
    }
